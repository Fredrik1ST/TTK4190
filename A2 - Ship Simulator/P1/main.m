...
%
% Author:           Group 1
% Study program:    Marine Technology & Cybernetics
...
control_law = 2;        % Choosing control law (rudder step or propeller step)
...
for i = 1:nTimeSteps
    ...
    % 4C Control laws
    if control_law == 1
         % --- Time-varying step response commands ---
        t_now   = t(i);
        delta_c = 0.1;   % [rad]
        n_c     = 10;   % [rps]
    
        % Step 1: rudder step (100–300 s)
        if t_now >= 300 && t_now < 800
            delta_c = 0.5;   % 5 deg rudder step
            n_c     = 10;            % propeller off
        end
    elseif control_law == 2
        ...
        % Step 2: propeller step (600–800 s)
        if t_now >= 300 && t_now < 800
            delta_c = 0.1;            % rudder neutral
            n_c     = 20;          % propeller speed step (rps)
        end
